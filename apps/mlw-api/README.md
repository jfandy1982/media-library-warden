# mlw-api (NestJS Backend)

Backend API for the Media Library Warden tool.

## Purpose

The `mlw-api` application serves as the backend for the Media Library Warden tool. It provides:
...
# Test with coverage
nx test mlw-api --coverage
```

### E2E Tests

End-to-end tests are located in `apps/mlw-api-e2e`.
...
```bash
nx g @nx/nest:application --directory=apps/mlw-api --linter=eslint --name=mlw-api --strict=true --tags="type:app, domain:api" --unitTestRunner=jest --e2eTestRunner=jest --useProjectJson=true
```
...
- **Frontend:** See [apps/mlw-ui-ng/README.md](../mlw-ui-ng/README.md) for Angular UI

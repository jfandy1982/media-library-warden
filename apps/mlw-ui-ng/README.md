# mlw-ui-ng (Angular Material Frontend)

User interface for the Media Library Warden tool built with Angular 21.1 and Material Design.

## Purpose

The `mlw-ui-ng` application provides the web-based user interface for the Media Library Warden tool. It enables users to:
...
This frontend communicates with the NestJS backend (`mlw-api`) which handles Emby Server integration and business logic.
...
- `npm run stack:ui:ng` starts UI only — requires `mlw-api` already running separately
...
```bash
nx g @nx/angular:component features/[component-name] --project=mlw-ui-ng --style=scss
```

**Create a new service:**

```bash
nx g @nx/angular:service features/[service-name] --project=mlw-ui-ng
```
...
- Displays: "Media Library Warden (Angular Material UI)"
...
# Test with coverage
nx test mlw-ui-ng --coverage

# Watch mode for development
nx test mlw-ui-ng --watch
```
...
**Cypress E2E (`apps/mlw-ui-ng-cypress-e2e`):**

```bash
# Run Cypress E2E tests (local, sequential on port 4200)
nx e2e mlw-ui-ng-cypress-e2e

# Run in CI mode with coverage (port 4201, requires build)
nx e2e-ci mlw-ui-ng-cypress-e2e --configuration=e2e-cypress-coverage
```

**Cucumber BDD E2E (`apps/mlw-ui-ng-cucumber-e2e`):**

```bash
# Run Cucumber BDD tests
nx e2e mlw-ui-ng-cucumber-e2e
```

**E2E Test Locations:**

- Cypress: `apps/mlw-ui-ng-cypress-e2e/src/e2e/smoke.cy.ts`
- Cucumber: `apps/mlw-ui-ng-cucumber-e2e/`
...
nx build mlw-ui-ng --configuration=production
...
- **Development:** `nx build mlw-ui-ng --configuration=development`
- **Production:** `nx build mlw-ui-ng --configuration=production`
...
- All components use the prefix `mlw-ng-` (e.g., `<mlw-ng-root>`)
- Configured in project settings

**App Creation Command:**

```bash
nx g @nx/angular:application --directory=apps/mlw-ui-ng --name=mlw-ui-ng --backendProject=mlw-api --e2eTestRunner=cypress --unitTestRunner=jest --linter=eslint --port=4200 --prefix=mlw-ng --style=scss --bundler=webpack --tags="type:app, domain:mlw-ui-ng, ui:angular"
```

**E2E Setup:** The generated e2e app was renamed and copied to support both Cypress and Cucumber approaches. The `-e2e` suffix is important for Nx to detect these as e2e project types.
...
- **Backend:** See [apps/mlw-api/README.md](../mlw-api/README.md) for NestJS API

# mlw-api-e2e (API E2E Tests)

End-to-end tests for the `mlw-api` NestJS backend application.

## Purpose

This is a dedicated E2E testing application that validates the API endpoints of `mlw-api` through actual HTTP requests.
...
# Run in watch mode
nx e2e mlw-api-e2e --watch

# Run with coverage
nx e2e mlw-api-e2e --coverage
```

## Test Structure

```
apps/mlw-api-e2e/src/
├── mlw-api/
│   └── mlw-api.spec.ts        # API endpoint tests
└── support/
...
- Location: `src/mlw-api/mlw-api.spec.ts`
...
Place test files in `src/mlw-api/` directory with `.spec.ts` suffix:

```bash
# Example structure
src/mlw-api/
├── mlw-api.spec.ts           # General API tests
...
- Place in `src/mlw-api/` directory
...
```
src/mlw-api/
├── mlw-api.spec.ts              # General/health check
...
- **API Application:** See [apps/mlw-api/README.md](../mlw-api/README.md) for API implementation details

# mlw-ui-ng-cypress-e2e (Cypress E2E Tests)

End-to-end tests for the `mlw-ui-ng` Angular frontend using Cypress.
...
nx e2e mlw-ui-ng-cypress-e2e --spec=src/e2e/smoke.cy.ts
```
...
```bash
# Run in CI mode with coverage on port 4201
...
**TypeScript Support:** Add type definitions for custom commands in `support/commands.ts`:
...
**When to Use Cucumber (`mlw-ui-ng-cucumber-e2e`):**
...
```bash
# Run in CI mode with coverage (port 4201, requires prior build)
nx e2e-ci mlw-ui-ng-cypress-e2e --configuration=e2e-cypress-coverage

# Artifacts saved to:
# - cypress/mlw-ui-ng-cypress-e2e/screenshots/
# - cypress/mlw-ui-ng-cypress-e2e/videos/  (video: false by default)
```
...
- **Cucumber E2E:** See [apps/mlw-ui-ng-cucumber-e2e/README.md](../mlw-ui-ng-cucumber-e2e/README.md) for BDD approach
